...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Wed Jun 12 13:32:18 BRT 2019
  */
      ...
      switch (CUP$parser$act_num)
		...
		Expresion expresion = (Expresion)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
                  ...
                  if (symbol instanceof TypeFunction) {
                      semanticErrorManager.semanticDebug("Function "+expresion+" expresion code: "+expresion.getIntermediateCode());
                      cb.addQuadruples(expresion.getIntermediateCode());
                      cb.addQuadruple("MV", temp, tempExp);
                      cb.addQuadruple("MV", variables.getVariable(), temp);
                  } else {
                     semanticErrorManager.semanticDebug("variables "+variables+" expresion code: "+variables.getIntermediateCode());
                      cb.addQuadruples(expresion.getIntermediateCode());
                   ...
                   semanticErrorManager.semanticDebug("sentAsign "+sentAsign+" expresion code: "+sentAsign.getIntermediateCode());
                  RESULT = sentAsign;
      ...
      ScopeIF currentScope = null;
        ...
        for(SymbolIF symbol : scope.getSymbolTable().getSymbols()) {
            if(symbol instanceof SymbolVariable) {
               ...
               if (scope.getLevel() == 0) {
                semanticErrorManager.semanticDebug("Global address of "+variableSymbol.getName()+" is now "+globalAddress);
                variableSymbol.setAddress(globalAddress);
                globalAddress += variableSymbol.getType().getSize();
                semanticErrorManager.semanticDebug("Global address is now "+globalAddress);
            } else {						
                ...
                semanticErrorManager.semanticDebug(variableSymbol.getName()  + "Incremento el tam del subprograma: "+ function);
              }
              semanticErrorManager.semanticDebug("Var: "+ variableSymbol.getName() 
                      + " tipo: " +variableSymbol.getType().getName() + "scope: "+variableSymbol.getScope()
                      + " -------------> direccion " + variableSymbol.getAddress());
  
           } else if (symbol instanceof SymbolFunction) {
            semanticErrorManager.semanticDebug("Funcion: "+ symbol.getName() + " scope: " + symbol.getScope()
                              + " tipo: " +symbol.getType().getName());
            procedures.put(symbol.getName(), (SymbolFunction)symbol);
          } else if (symbol instanceof SymbolProcedure) {
            semanticErrorManager.semanticDebug("Procedimiento: "+ symbol.getName() + " scope: " + symbol.getScope()
                              + " tipo: " +symbol.getType().getName());
            ...
            semanticErrorManager.semanticDebug("Param: "+ parametro.getName() 
                              + " tipo: " +parametro.getType().getName()
                              + " -------------> direccion " + parametro.getAddress() + " Desplazamiento :" + scopeOffset);
            semanticErrorManager.semanticDebug("Incremento el tam del subprograma: "+ function +": " + function.getSize());
          } else {
            semanticErrorManager.semanticDebug("Simbolo no procesado: "+ symbol.getName() + " " + symbol.getClass());
          }
          ...
          for (TemporalIF t:temporalesAmbito) {
            if (scope.getLevel() == 0) {
              t.setAddress(scopeOffset);
                          semanticErrorManager.semanticDebug("-------Global adress change from " +scopeOffset+"---------------to "+scopeOffset+++"--------------------");
            } else {
            ...
            semanticErrorManager.semanticDebug("Temporal: "+ t.getName() + " " + t.getScope()
                                + " -------------> direccion " + t.getAddress());
          }

          if (scope.getLevel() == 0) {
            currentScope = scope;
            semanticErrorManager.semanticDebug("-------Global adress change from " +globalOffset+"---------------to "+scopeOffset+"--------------------");
            globalOffset = scopeOffset;
          }
			}

      for (SymbolProcedure p : procedures.values()) {
            semanticErrorManager.semanticDebug("Funciones y procedimiento: "+ p.getName() + " Size: " + p.getSize()
														+ "varTempSize: " + p.getTempSize());
          }
        ...
        semanticErrorManager.semanticDebug("-------Global adress " +globalAddress+"--------------------------------------");
        semanticErrorManager.semanticDebug("-------Global offset " +globalOffset+"--------------------------------------");
