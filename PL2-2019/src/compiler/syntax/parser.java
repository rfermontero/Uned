...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Thu May 16 23:13:49 BRT 2019
  */
    ...
    throws java.lang.Exception
    {
      ...
      switch (CUP$parser$act_num)
		...
		Token string = (Token)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
                ...
                listaCadenas.add(new Quadruple("STRING", new Label(string.getLexema()), lb)); 
		...
		Sentencias sentencias = (Sentencias)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
                  ...
                  TemporalIF temp = tf.create();
                  TemporalIF eTempFrom = expresionFrom.getTemporal();
                  TemporalIF e2TempTo = expresionTo.getTemporal();
		...
		SentElse sentElse = (SentElse)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
                  ...
                  TemporalIF expTemp = expresion.getTemporal();
                  IntermediateCodeBuilder cb= new IntermediateCodeBuilder(getCurrentScope());
                  cb.addQuadruples(expresion.getIntermediateCode());
                  cb.addQuadruple("BRF", expTemp, etiquetaElse);
                  cb.addQuadruples(sentencias.getIntermediateCode());
                  cb.addQuadruple("BR", etiquetaFinIf);
                  cb.addQuadruple("INL", etiquetaElse);
                  cb.addQuadruples(sentElse.getIntermediateCode());
                  cb.addQuadruple("INL", etiquetaFinIf);
		...
		Axiom ax = (Axiom)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
			...
			/* Dirección inicial para variables globales, las dos primeras posiciones quedan
			ocupadas con la sentencia inicial de salto al código del programa */
			int gAddress = 2;
			/* Desplazamiento relativo inicial para variables locales y parámetros, las tres
			primeras posiciones de desplazamiento están ocupadas por el valor de retorno, el
			enlace de control y el estado de la máquina del registro de activación del subprograma */
			int lOffset = 3;
			String scopeName = new String();
			// Listamos todos los ámbitos del programa
			List<ScopeIF> scopes = scopeManager.getAllScopes();
			// y los recorremos
			for (ScopeIF scope : scopes) {
				// Si el ámbito es de nivel 1 reseteamos el desplazamiento relativo a 3
				if (scope.getLevel() == 1) {
					scopeName = scope.getName();
					lOffset = 3;
				}
				/* Asignamos el desplazamiento relativo a los parámetros de los subprogramas en el mismo
				orden en que han sido declarados */
			
				/* Si el ámbito es de un subprograma incrementamos el desplazamiento en 1 para dejar
				hueco a la dirección de retorno, en el registro de activación */
				if (scope.getLevel() == 1) {
					lOffset++;
				}
				// Creamos un nuevo constructor de código intermedio
				IntermediateCodeBuilderIF cb = new IntermediateCodeBuilder(scope);
				// Recuperamos los símbolos de la tabla de símbolos del ámbito
				List<SymbolIF> symbols = scope.getSymbolTable().getSymbols();
				// Para cada símbolo de tipo SymboVariable asignamos su dirección o desplazamiento
				for (SymbolIF symbol : symbols) {
					if (symbol instanceof SymbolVariable) {
						SymbolVariable sv = (SymbolVariable) symbol;
						// Si el ámbito es global asignamos una dirección global
						if (scope.getLevel() == 0) {
							sv.setAddress(gAddress);
							int value = sv.getValue();
							// Si la variable es de tipo simple la inicializamos
							if (value != -1) {
								Variable var = new Variable(sv);
								cb.addQuadruple("INITVAR",var,value);
							}
							gAddress += sv.getType().getSize();
						// Si el ámbito no es global asignamos un desplazamiento relativo
						} else {
							sv.setAddress(lOffset);
							lOffset += sv.getType().getSize();
						}
					}
				}
				// Recuperamos la lista de temporales del ámbito
				List<TemporalIF> temporals = scope.getTemporalTable().getTemporals();
				// Para cada temporal asignamos un deplazamiento relativo
				for (TemporalIF t : temporals) {
					if (t instanceof Temporal) {
						((Temporal) t).setAddress(lOffset);
						lOffset++;
					}
				}
				// Añadimos cuádruplas y generamos el código intermedio
				cb.addQuadruples(ax.getIntermediateCode());
				ax.setIntermediateCode(cb.create());
			}

			// Recuperamos el ámbito actual
			ScopeIF s = scopeManager.getCurrentScope();
			// Creamos un nuevo contructor de código intermedio
			IntermediateCodeBuilderIF cb = new IntermediateCodeBuilder(s);
			// Creamos una fabrica de etiquetas
			LabelFactoryIF lf = new LabelFactory();
			//Creamos etiqueta
			LabelIF l1 = lf.create("codigo");
			/* Instrucción de código intermedio que salta a la ubicación del código
			del programa */
			cb.addQuadruple("BR",l1);
			// Dejamos hueco en la memoria para ubicar las variables globales
			cb.addQuadruple("ORG",new Value(gAddress));
			// Insertamos la etiqueta que marca el inicio del código del programa
			cb.addQuadruple("INL",l1);
			// Añadimos el código intermedio del programa
			cb.addQuadruples(ax.getIntermediateCode());
			// Añadimos las cúadruplas de codigo intermedio de las cadenas del programa
			cb.addQuadruples(listaCadenas);
			// Creamos el código intermedio resultante
			ax.setIntermediateCode(cb.create());
      ...
      // Impresión por pantalla del código intermedio generado para el programa
			System.out.println("** CODIGO INTERMEDIO: **");
			for (QuadrupleIF q : ax.getIntermediateCode()) {
				System.out.println(q);
			}
			System.out.println("** FIN CODIGO INTEREMEDIO **");
